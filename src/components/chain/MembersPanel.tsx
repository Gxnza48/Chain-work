...
export function MembersPanel({ chainId, members, myRole, onChanged }: Props) {
  ...
  async function promote(member: ChainMemberProfile) {
    ...
    if (error) {
      toast.error(t('Could not promote'), { description: error.message });
      return;
    }
    toast.success(t('{name} is now an owner', { name: member.display_name }));
    onChanged?.();
  }

  async function ban(member: ChainMemberProfile) {
    if (!window.confirm(t('Remove {name} from this chain?', { name: member.display_name }))) return;
    const { error } = await supabase
      .from('chain_members')
      .delete()
      .eq('chain_id', chainId)
      .eq('user_id', member.id);
    if (error) {
      toast.error(t('Could not remove member'), { description: error.message });
      return;
    }
    toast.success(t('{name} was removed', { name: member.display_name }));
    onChanged?.();
  }
  ...
  function itemsFor(member: ChainMemberProfile): ContextMenuItem[] {
    ...
    return [
      {
        label: t('Promote to owner'),
        icon: <Crown className="h-4 w-4" />,
        onSelect: () => promote(member),
      },
      {
        label: t('Ban from chain'),
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-4 pt-4">
        <h2 className="font-display text-sm font-bold uppercase tracking-[0.18em] text-fg-muted">{t('Members')}</h2>
        <p className="mt-0.5 text-xs text-fg-muted">
          {t('{count} total', { count: members.length })}
          {isOwner ? t(' · right-click to manage') : ''}
        </p>
      </div>
      <ul className="mt-3 flex-1 overflow-y-auto px-2 pb-4">
        ...
        {members.length === 0 ? (
          <li className="px-2 py-6 text-sm text-fg-muted">{t('No members yet.')}</li>
        ) : null}
      </ul>
      {isOwner ? (
