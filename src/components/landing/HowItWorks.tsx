...
export function HowItWorks() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const cards = sectionRef.current?.querySelectorAll<HTMLElement>('[data-step]') ?? [];
      gsap.from(cards, {
        opacity: 0,
        y: 30,
        duration: 0.6,
  ...
  return (
    <section
      id="how-it-works"
      ref={sectionRef}
      className="relative scroll-mt-28 py-24 md:py-32"
    >
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <li
                key={step.title}
                data-step
                className="brut-card flex flex-col gap-4 p-6 will-change-transform"
              >
